...
import { buildTopicPrompt, narrowBroadTopic, conceptPlan, timesTablePlan, numberBondsPlan, shapesPlan, fractionsPlan, comparePlan, oddEvenPlan, placeValuePlan, countingPlan, moneyPlan, spagPlan, rhymingPlan, cvcPlan, daysMonthsPlan, romanNumeralsPlan, multiplesFactorsPlan } from '@/lib/topic-prompt'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const plan =
      ...
      timesTablePlan(topic, age) ||
      numberBondsPlan(topic, age) ||
      shapesPlan(topic, age) ||
      fractionsPlan(topic, age) ||
      comparePlan(topic, age) ||
      oddEvenPlan(topic, age) ||
      placeValuePlan(topic, age) ||
      moneyPlan(topic, age) ||
      countingPlan(topic, age) ||
      spagPlan(topic, age) ||
      rhymingPlan(topic, age) ||
      cvcPlan(topic, age) ||
      multiplesFactorsPlan(topic, age) ||
      daysMonthsPlan(topic, age) ||
      romanNumeralsPlan(topic, age) ||
